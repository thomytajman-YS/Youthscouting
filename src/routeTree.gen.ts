/* eslint-disable */

// @ts-nocheck

// noinspection JSUnusedGlobalSymbols

// This file was automatically generated by TanStack Router.
// You should NOT make any changes in this file as it will be overwritten.
// Additionally, you should also exclude this file from your linter and/or formatter to prevent it from being checked or modified.

import { Route as rootRouteImport } from './routes/__root'
import { Route as ExploradorRouteImport } from './routes/explorador'
import { Route as EstadisticasRouteImport } from './routes/estadisticas'
import { Route as IndexRouteImport } from './routes/index'
import { Route as JugadorIdRouteImport } from './routes/jugador.$id'

const ExploradorRoute = ExploradorRouteImport.update({
  id: '/explorador',
  path: '/explorador',
  getParentRoute: () => rootRouteImport,
} as any)
const EstadisticasRoute = EstadisticasRouteImport.update({
  id: '/estadisticas',
  path: '/estadisticas',
  getParentRoute: () => rootRouteImport,
} as any)
const IndexRoute = IndexRouteImport.update({
  id: '/',
  path: '/',
  getParentRoute: () => rootRouteImport,
} as any)
const JugadorIdRoute = JugadorIdRouteImport.update({
  id: '/jugador/$id',
  path: '/jugador/$id',
  getParentRoute: () => rootRouteImport,
} as any)

export interface FileRoutesByFullPath {
  '/': typeof IndexRoute
  '/estadisticas': typeof EstadisticasRoute
  '/explorador': typeof ExploradorRoute
  '/jugador/$id': typeof JugadorIdRoute
}
export interface FileRoutesByTo {
  '/': typeof IndexRoute
  '/estadisticas': typeof EstadisticasRoute
  '/explorador': typeof ExploradorRoute
  '/jugador/$id': typeof JugadorIdRoute
}
export interface FileRoutesById {
  __root__: typeof rootRouteImport
  '/': typeof IndexRoute
  '/estadisticas': typeof EstadisticasRoute
  '/explorador': typeof ExploradorRoute
  '/jugador/$id': typeof JugadorIdRoute
}
export interface FileRouteTypes {
  fileRoutesByFullPath: FileRoutesByFullPath
  fullPaths: '/' | '/estadisticas' | '/explorador' | '/jugador/$id'
  fileRoutesByTo: FileRoutesByTo
  to: '/' | '/estadisticas' | '/explorador' | '/jugador/$id'
  id: '__root__' | '/' | '/estadisticas' | '/explorador' | '/jugador/$id'
  fileRoutesById: FileRoutesById
}
export interface RootRouteChildren {
  IndexRoute: typeof IndexRoute
  EstadisticasRoute: typeof EstadisticasRoute
  ExploradorRoute: typeof ExploradorRoute
  JugadorIdRoute: typeof JugadorIdRoute
}

declare module '@tanstack/react-router' {
  interface FileRoutesByPath {
    '/explorador': {
      id: '/explorador'
      path: '/explorador'
      fullPath: '/explorador'
      preLoaderRoute: typeof ExploradorRouteImport
      parentRoute: typeof rootRouteImport
    }
    '/estadisticas': {
      id: '/estadisticas'
      path: '/estadisticas'
      fullPath: '/estadisticas'
      preLoaderRoute: typeof EstadisticasRouteImport
      parentRoute: typeof rootRouteImport
    }
    '/': {
      id: '/'
      path: '/'
      fullPath: '/'
      preLoaderRoute: typeof IndexRouteImport
      parentRoute: typeof rootRouteImport
    }
    '/jugador/$id': {
      id: '/jugador/$id'
      path: '/jugador/$id'
      fullPath: '/jugador/$id'
      preLoaderRoute: typeof JugadorIdRouteImport
      parentRoute: typeof rootRouteImport
    }
  }
}

const rootRouteChildren: RootRouteChildren = {
  IndexRoute: IndexRoute,
  EstadisticasRoute: EstadisticasRoute,
  ExploradorRoute: ExploradorRoute,
  JugadorIdRoute: JugadorIdRoute,
}
export const routeTree = rootRouteImport
  ._addFileChildren(rootRouteChildren)
  ._addFileTypes<FileRouteTypes>()

import type { getRouter } from './router.tsx'
import type { startInstance } from './start.ts'
declare module '@tanstack/react-start' {
  interface Register {
    ssr: true
    router: Awaited<ReturnType<typeof getRouter>>
    config: Awaited<ReturnType<typeof startInstance.getOptions>>
  }
}
